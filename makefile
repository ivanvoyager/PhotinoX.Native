...
FULL_VERSION=$(VER_MAJOR).$(VER_MINOR).$(VER_PATCH).$(VER_BUILD)
VERSION_HEADER=$(SRC)/version.h

generate-version-header:
	@printf '%s\n' \
		'#pragma once' \
		'// Auto-generated by make. Do not edit manually.' \
		'// Generated on: $(GENERATED_ON)' \
		'' \
		'#define VER_STR "$(FULL_VERSION)"' \
		'#define YEAR_STR "$(COPYRIGHT_YEAR)"' \
		> $(VERSION_HEADER)
	@echo "Generated version.h: $(FULL_VERSION) (Copyright $(COPYRIGHT_YEAR))"
...
.PHONY: all windows mac-universal linux-x64 linux-arm64 \
        ...
        generate-version-header install-linux-dependencies clean-x64 clean-arm64
...
build-photino-mac-universal: generate-version-header | $(DEST_PATH_X64)
...
# Linux x64 .so
build-photino-linux-x64: generate-version-header | $(DEST_PATH_X64)
	$(CXX) $(CXXFLAGS) $(SOFLAGS) $(LDFLAGS) \
...
build-photino-linux-arm64: generate-version-header | $(DEST_PATH_ARM64)
	$(CXX) $(CXXFLAGS) $(SOFLAGS) $(LDFLAGS) \
