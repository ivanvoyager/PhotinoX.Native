...
| **Window initialization and visibility** | Native window creation and initial display are coupled to the original startup path.                                                                                                 | Supports initializing the native window and WebView without showing the top-level window, while preserving the existing show-on-initialization path. Initial window state is applied when the window is first shown, and initialized windows can be hidden and shown without being destroyed.                         |
