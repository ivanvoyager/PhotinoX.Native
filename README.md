...
| **Application / message loop model** | Uses global native wait/invoke helpers around the native window/message loop. | Introduces `PhotinoApplication` as the native application lifetime object, with explicit run/shutdown semantics, startup, shutdown-requested, and exit callbacks, check-access support, and synchronous/asynchronous dispatch. |
