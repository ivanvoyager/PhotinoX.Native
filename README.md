...
PhotinoX.Native provides a lightweight native window host using each OS’s built‑in WebView stack:
...
|---------|--------------|------------------|----------------------------------------|
| Windows | x64          | `win-x64`        | `Photino.Native.dll`, `WebView2Loader.dll` |
| Windows | ARM64        | `win-arm64`      | `Photino.Native.dll`, `WebView2Loader.dll` |
| Linux   | x64          | `linux-x64`      | `Photino.Native.so`                    |
| Linux   | ARM64        | `linux-arm64`    | `Photino.Native.so`                    |
| macOS   | x64          | `osx-x64`        | `Photino.Native.dylib` (universal)     |
| macOS   | ARM64        | `osx-arm64`      | `Photino.Native.dylib` (universal)     |

All files conform to the standard NuGet `runtimes/<rid>/native/` layout.

This package is intended for developers building modern desktop apps with  
web‑based UI technologies (Blazor, React, Vue, Angular, etc.) on top of  
